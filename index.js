...
module.exports = function HiWelcomeToMyMinecraftLetsPlayImYourHostHUUUUUUUUUUGEEEEDOOOONNNG(dispatch) {
    ...
    dispatch.hook('S_JOIN_PRIVATE_CHANNEL', 2, event => event.index === config.chatChannel - 1 ? false : undefined);
    dispatch.hook('C_LEAVE_PRIVATE_CHANNEL', 1, event => event.index === config.chatChannel - 1 ? false : undefined);
    dispatch.hook('C_REQUEST_PRIVATE_CHANNEL_INFO', 2, (event) => {
        if (event.chatChannel === PRIVATE_CHANNEL_ID) {
    ...
    dispatch.hook('C_CHAT', 1, { order: -9 }, (event) => {
        if (event.channel === config.chatChannel + 10) {
